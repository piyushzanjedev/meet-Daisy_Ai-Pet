...
def get_user_data_dir() -> str:
    """Returns the persistent directory for user configuration and keys."""
    if getattr(sys, "frozen", False):
        ...
        try:
            ...
            daisy_dir = os.path.join(appdata, "Daisy")
...
def _fetch_active_groq_models(key: str, timeout: int = 10) -> List[str]:
    """Dynamically queries Groq's models endpoint for active chat models."""
    try:
        resp = requests.get(
        ...
        if resp.status_code == 200:
            ...
            for item in data:
                m_id = item.get("id", "")
                if item.get("active", True) and not any(skip in m_id.lower() for skip in ["whisper", "prompt-guard", "guard"]):
...
def _ask_groq(question: str, key: str, timeout: int = 20, system_prompt: Optional[str] = None) -> str:
    ...
    for model in models_to_try:
        ...
        # If model is not found, decommissioned, or 404, continue to next model
        if (
            status_code == 404
            or err_code in ("model_decommissioned", "model_not_found")
    ...
    # If static candidates were all decommissioned/unavailable and no custom model forced, try dynamic model discovery
    if not custom_model:
        active_models = _fetch_active_groq_models(key, timeout=min(timeout, 8))
